...
public class Ex67_Lamda {
	public static void main(String[] args) {
		...
		/*
			...
			람다식 > 익명 객체의 추상 메서드 > 인터페이스 참조 변수에 저장 > 람다식을 사용하려면 반드시 인터페이스가 필요하다.!!
			
			람다식의 가장 큰 장저 > 간단함(생산성)
			람다식의 가장 큰 단점 > 인터페이스를 선언해야 한다;;
			
			*** 자바에서는 여러가지 형식의 람다식(추상 메서드)를 저장할 수 있는 인터페이스를 미리 제공 > 함수형 인터페이스(Functional Interface)
	...
	private static void m4() {
		//Operator
		//- 매개변수를 전달하면 처리후 반환값을 돌려주는 업무를 구현하는 인터페이스
		//- applyXXX() 추상 메서드 제공 
	...
	private static void m1() {
		...
		c5.accept(null);new Member("홍길동", 20);
		...
		BiConsumer<Integer, Integer> bc2 = (a,b)-> System.out.printf("이름: %s, 나이: %d세\n",a, b);
		bc2.accept(10, 20);
