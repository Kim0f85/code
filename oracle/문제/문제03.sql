...
select 
    count(*)
from tblInsa
    where (ssn like '__07__-2%' or ssn like '__08__-2%' or ssn like'__09__-2%'); 
...
select 
    round(avg(case when jikwi in ('부장','과장') then basicpay end),2) as 장,
    round(avg(case when jikwi in ('대리','사원') then basicpay end),2) as 원,
    '장'-'원'
from tblInsa
