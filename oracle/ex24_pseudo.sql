...
select name, buseo, rownum from tblInsa where rownum = 5;
select name, buseo, rownum from tblInsa where rownum > 5 and jrownum <= 10;

-- *** 1. rownum은 from절이 호출될 때 계산되어진다. 
-- *** 2. where절에 의해 결과셋의 변화가 발생될때 다시 계산되어진다.


select  name,buseo, rownum 
from tblInsa;               --1. from절이 실행되는 순간 모든 레코들에 rownown을 가져온다
